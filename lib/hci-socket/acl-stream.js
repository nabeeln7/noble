...
const AclStream = function (hci, handle, localAddressType, localAddress, remoteAddressType, remoteAddress) {
  ...
  this.onSmpBondingBinded = this.onSmpBonding.bind(this);
  ...
  this._smp.on('bondingInfo', this.onSmpBondingBinded);
...
AclStream.prototype.onSmpBonding = function (bondingInfo) {
  const ltk = bondingInfo['ltk'];
  const random = bondingInfo['rand'];
  const diversifier = bondingInfo['ediv'];

  this._hci.startLeEncryption(this._handle, random, diversifier, ltk);
};
